...
function edit({ complaint, customers, products, employees }) {
    ...
    const { put, data, setData, errors, processing } = useForm({
        ...
        assigned_id: complaint.assigned_id,
    ...
    function handleSubmit(e) {
        e.preventDefault();
        put(route('complaint.update',complaint.id), {
            onSuccess: () => {
                // Show success notification on successful submission
                notyf.success('complaint updated successfully!');
            },
            onError: () => {
    ...
    return (
        <AdminLayout>
            <div className="max-w-5xl p-8 mx-auto bg-white rounded-lg shadow-md">
                <h2 className="mb-6 text-2xl font-bold text-gray-800">Edit Complaint</h2>
                <form onSubmit={handleSubmit} className="flex flex-wrap">
                    ...
                    <div className='w-full py-5'>
                        <button className='py-2 px-6 text-sm bg-rose-600 rounded font-medium text-white'>Edit Complaint</button>
                    </div>
                </form>
            </div>
        </AdminLayout>
