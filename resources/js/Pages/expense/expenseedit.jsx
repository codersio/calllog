...
const ExpenseEditPage = ({ taxes, products, employees,customers,expense }) => {
const { data, setData, post, errors } = useForm({
    payee_type: expense.payee_type || 'Employee',
    ...
    items: expense.items.length > 0
        ...
        : [{ service_id: '', qty: '', price: '', discount: '', tax: '' }]
});
            ...
            <div className="p-6 mx-auto max-w-7xl">
                <form onSubmit={handleSubmit}>
                    <div className="p-6 bg-white rounded shadow">
                        <h1 className="mb-4 text-xl font-bold">Expense Create { expense.payee_id }</h1>
...
{data.payee_type === "Employee" && (
   <select
    value={employees.some(employee => employee.id === data.payee_id) ? data.payee_id : ""}
...
{data.payee_type === "Customer" && (
    <select
        value={customers.some(customer => customer.id === data.payee_id) ? data.payee_id : ""}
        onChange={(e) => setData('payee_id', e.target.value)}
        className="w-full p-2 border rounded"
    >
...
{data.payee_type === "Vendor" && (
    <select
        value={vendors.some(vendor => vendor.id === data.payee_id) ? data.payee_id : ""}
        onChange={(e) => setData('payee_id', e.target.value)}
        className="w-full p-2 border rounded"
    >
