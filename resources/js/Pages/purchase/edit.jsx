...
function edit({ suppliers, products, taxes, pur }) {

    const [rows, setRows] = useState(pur.sales_details);
    ...
    const { put, data, setData, errors, processing } = useForm({
        ...
        status: pur.status,
        date: pur.date,
        contact_person: pur.contact_person,
        supplier_id: pur.supplier_id,
        email: pur.email,
        billing_address: pur.billing_address,
        sales_details: [],
    })
    ...
    function handleSubmit(e) {
        e.preventDefault();
        put(route('purchase.update',pur.id), {
            onSuccess: () => {
                notyf.success('Purchase updated successfully!');
            },
            onError: () => {
                notyf.error('Failed to update purchase. Please check your inputs.');
            }
        })
    }
    return (
        <AdminLayout>
            <div className="max-w-5xl p-8 mx-auto bg-white rounded-lg shadow-md">
                <h2 className="mb-6 text-2xl font-bold text-gray-800">Edit Purchase</h2>
