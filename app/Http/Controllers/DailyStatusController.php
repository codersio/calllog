...
class DailyStatusController extends Controller
{
    ...
    public function tasktime(DailyStatus $dailyStatus)
    {
        $tasksWithTimesheets = Timesheet::join('tasks', 'tasks.id', '=', 'timesheets.task_id')
            ...
            ->where('timesheets.user_id', Auth::id())
            ->select(
                'task_assigns.task_id',  // Group by task id from task_assigns
                'tasks.task_name',
                DB::raw('MAX(task_assigns.employee_hours) as estimate_hours'),  // Use MAX or MIN to handle multiple task_assigns
                DB::raw('SUM(timesheets.time_number) as total_time_worked')
            )
            ->groupBy('task_assigns.task_id', 'tasks.task_name')  // Group by task_id, and task_name only
            ->get();
